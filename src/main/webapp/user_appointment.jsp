			...
			<div class="col-md-6">
				<div class="card paint-card">
					<div class="card-body">
						...
						<c:if test="${not empty suchMsg}">
							<p class=" fs-4 text-center text-success">${suchMsg}</p>
							<c:remove var="suchMsg" scope="session" />
						</c:if>
						<form class="row g-3" action="appAppointment" method="post">
							...
							<div class="col-md-6">
								<label for="inputPassword4" class="form-label">Doctor</label> <select
									...
									for (Doctor d : list) {
									%>
									<option value="<%=d.getId()%>"><%=d.getId() %>-<%=d.getSpecialist()%>
									</option>
									<%
									}
									%>
							...
							<c:if test="${empty userObj }">
								<a href="UserLogin.jsp" class="col-md-6 offset-md-3 btn btn-info">Submit</a>
							</c:if>
